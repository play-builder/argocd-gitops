...
def validate_db04(selected):
    ...
    def release_identity(value, label, recovered=False):
        ...
        if image_match is None or image_match.group("account") != account_id or image_match.group("region") != region or not 2 <= len(image_match.group("name")) <= 256 or re.search(r"(?:^|/)mini-commerce$", image_match.group("name")) is None:
    ...
    for scenario in ("git-revert", "break-glass-undo-plus-git", "hotfix-fix-forward"):
        ...
        if any(recovery[name]["imageRepository"] != recovery["stable"]["imageRepository"] for name in ("faulty", "recovered")):
            fail(f"INC-DB-04/{scenario} image repositories must match the stable mini-commerce identity")
