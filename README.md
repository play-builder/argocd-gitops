...
Istio desired state is staged as five manual Applications per cluster: shared base 1.31.0, stable istiod/gateway 1.30.4, and candidate istiod/gateway 1.31.0. Each tag has one chart owner. Gateway API Standard 1.6.0 CRDs remain an EKS prerequisite. A separate `platform-mesh-*` project confines Helm ConfigMaps/RBAC to mesh namespaces and cannot access `argocd`. The root owns only the new `istio-system` namespace at wave -36; current application namespace ownership remains unchanged.

Batch 2 preserves the existing Gateway Rollouts plugin and traffic. The new ALB edge is a parallel, unactivated path; it terminates public HTTPS:443 using ACM and forwards internal HTTP:80 to the Istio ingress. Its WAF reference is `spec.wafV2.webACL`. No management port is a route backend. Native VirtualService routing is Task 9, so the new edge does not yet deliver application traffic. Keep DNS and existing edge unchanged. Do not sync the restrictive mesh policy Application or activate `argocd/overlays/mesh-enable` before the Task 9 traffic/identity/network-policy transition is ready. Prod RDS egress awaits the exact private endpoint/CIDR contract in Task 10; no speculative DB network access is opened.

Before activation, replace WAF/ACM/host references with approved inputs and run `ruby scripts/validate-mesh-inputs.rb RENDERED-MESH.yaml`; committed WAF/ACM placeholders deliberately fail this gate. Verify EKS Gateway CRDs/ESO/Sigstore readiness, sync base then stable/candidate control planes then gateways. Dev has a separately rendered PERMISSIVE migration overlay, ending in STRICT. Remove any legacy injection label, select the explicit revision tag through the existing namespace owner, and restart only approved workloads. ADOT must be injected with the declared collector ServiceAccount for authenticated mesh scraping. Confirm probe rewriting and ports 15020/15021, proxy SYNCED, mTLS/Authz, service traffic and SLOs in the cluster. Only then transition the route in Task 9. Tag rollback requires reinjection; stable revision retirement requires old-proxy count zero, rollback evidence and explicit approval. None of those live gates has run.

Source verification uses Argo CD 3.5 Git GPG `head` mode. Replace the explicit key-ID placeholder with an approved EKS-imported key before sync. `strict` requires signed seal, key propagation and real sync evidence. Helm/OCI archives are independently verified by `scripts/validate-rendered-manifests.sh`; it consumes every archive lock and pinned CRDs, and rejects unknown Rollout fields. Upstream CRD definitions supply schemas and are reported separately from validated workload instances.
